...
model {
  ...
  rho_gp ~ normal(0,80000) T[1e-3,1e10]; // normal(10000,50000)
  alpha_gp ~ beta(1,2) T[0,1]; 
  // sigma_gp ~  normal(0,0.05); // inv_gamma(0.01,0.03) T[1e-3,1e3];  //inv_gamma(0.01,0.03) T[1e-3,3]; 
    sigma_gp ~  inv_gamma(0.05,0.05); // inv_gamma(0.01,0.03) T[1e-3,1e3];  //inv_gamma(0.01,0.03) T[1e-3,3]; 
  ...
  for(i in 1:n_obsvar) {
    // sigma_obs[i] ~ normal(0,2); //inv_gamma(0.01,0.1)  // observation var sigma
        sigma_obs[i] ~ inv_gamma(0.05,0.2); //inv_gamma(0.01,0.1)  // observation var sigma
  }
