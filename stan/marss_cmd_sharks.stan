...
model {
  ...
  for(i in 1:n_obsvar) {
    sigma_obs[i] ~ inv_gamma(0.05,0.2) ;//student_t(5,0,0.1); // observation var sigma
  }
  for(s in 1:n_provar) {
    sigma_process[s] ~ inv_gamma(0.04,0.04)  ; // inv_gamma(0.01,0.03) T[1e-4,1e2]; //inv_gamma(0.01,0.03) T[1e-3,1]; //student_t(5,0,0.1); // process var sigma
