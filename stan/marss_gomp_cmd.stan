...
parameters {
  ...
  vector<lower=0>[S] gomp_int; // initial states
}
transformed parameters {
  vector[S] K;
  for(s in 1:S) {K[s] = gomp_int[s]/gomp_dd[s];}
  ...
  for(t in 2:N) {
    ...
    x[t,s] = x[t-1,s] + pro_dev[t-1,s] * sigma_process[proVariances[s]] + gomp_int[s] - gomp_dd[s]*x[t-1,s]; // * sigma_obs[1]; 
    if(est_trend == 1) {
     x[t,s] = x[t,s] + Uvec[s];
    }
    }
...
model {
  ...
  for(i in 1:n_obsvar) {
    sigma_obs[i] ~ normal(0,2);//student_t(5,0,0.1); // observation var sigma
  }
  for(s in 1:n_provar) {
    sigma_process[s] ~ normal(0,0.05); // inv_gamma(0.01,0.03) T[1e-4,1e2]; //inv_gamma(0.01,0.03) T[1e-3,1]; //student_t(5,0,0.1); // process var sigma
